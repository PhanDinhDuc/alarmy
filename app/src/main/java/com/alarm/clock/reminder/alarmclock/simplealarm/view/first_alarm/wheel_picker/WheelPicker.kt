package com.alarm.clock.reminder.alarmclock.simplealarm.view.first_alarm.wheel_picker
...
interface OnValueTapListener {
     ...
     * @param newVal The new value.
     */
    fun onValueTap(picker: WheelPicker, value: String)
}
...
class WheelPicker @JvmOverloads constructor(
    ...
    private var mOnValueTapListener: OnValueTapListener? = null
    ...
    init {
        ...
        mSelectedTextColor = attributesArray.getColor(
            ...
            ContextCompat.getColor(context, R.color.textPrimaryColor)
        )
        mUnSelectedTextColor = attributesArray.getColor(
            R.styleable.WheelPicker_textColor, ContextCompat.getColor(context, R.color.newtral_3)
        )
    ...
    private fun handlerClickVertical(y: Int) {
        ...
        if (selectorIndexOffset == 0) {
            mOnValueTapListener?.onValueTap(this, getCurrentItem())
        }
...
//            if (!mIsDragging)
    ...
    private fun adjustItemVertical() {
        ...
        if (deltaY != 0) {
            mOverScroller!!.startScroll(scrollX, scrollY, 0, deltaY, 250)
            invalidateOnAnimation()
        }
    ...
    private fun drawVertical(canvas: Canvas) {
        ...
        while (i < mSelectorItemIndices.size) {
            ...
            val offsetToMiddle =
                Math.abs(y - (mInitialFirstItemOffset + mWheelMiddleItemIndex * itemHeight).toFloat())
            ...
            if (mSelectorItemValidStatus[i]) {
                ...
                mTextPaint.color = ContextCompat.getColor(context, R.color.textPrimaryColor)
            }
    ...
    fun setOnValueTapListener(onValueTapListener: OnValueTapListener) {
        mOnValueTapListener = onValueTapListener
    }
    ...
    fun reload() {
        val currentValue = getCurrentItem()
        reset()
